# =============================================================================
# covid19_nl_wave2.R
#
# Wave 2: 7 September 2020 – 17 January 2021
# Dominant variant: ancestral strain
# Context: autumn resurgence; expanded testing; gradual tightening of NPIs;
#          full lockdown on 14 December 2020 coincides with epidemic peak.
#
# Differences from Wave 1:
#   - Wave starts ALREADY at 12,710 cases/week (no small seed)
#   - Slow growth (Rt ≈ 1.16) over 14 weeks before peak
#   - Only two NPI phases: slow growth → full lockdown (no reopening in window)
#     Beta3 = Beta2 (both set to post-lockdown value)
#   - rho = 0.18 (higher than W1; expanded testing)
#   - eta = 0.95 (5% immune from Wave 1 seroprevalence)
#   - I0, E0 much larger (epidemic already active at wave start)
# =============================================================================

library(tidyverse)
library(pomp)
library(coda)
library(GGally)

set.seed(123456)

# ─────────────────────────────────────────────────────────────────────────────
# 1.  DATA
# ─────────────────────────────────────────────────────────────────────────────

setwd("C:/Users/Usuario/Desktop/CBS - Internship/packages")
daily_raw <- read_csv("covid19_daily_cases_NL.csv") |>
  mutate(date = as.Date(date))

weekly <- daily_raw |>
  mutate(year_week = floor_date(date, unit = "week", week_start = 1)) |>
  group_by(year_week) |>
  summarise(reports = sum(cases), .groups = "drop") |>
  arrange(year_week)

meas_w2 <- weekly |>
  filter(year_week >= as.Date("2020-09-07"),
         year_week <= as.Date("2021-01-17")) |>
  mutate(week = seq_len(n())) |>
  select(week, reports)

cat("Wave 2:", nrow(meas_w2), "weekly obs | peak:", max(meas_w2$reports),
    "(wk", which.max(meas_w2$reports), ") | total:", sum(meas_w2$reports), "\n")

# ─────────────────────────────────────────────────────────────────────────────
# 2.  NPI COVARIATE
# ─────────────────────────────────────────────────────────────────────────────
# Wave 2 NPI timeline (relative to wave start, 7 Sep 2020):
#   Phase 0 (t < 14): gradual tightening measures — partial closures (Sep 29),
#                     curfew (Oct 14), stricter rules (Nov 4). Epidemic still
#                     growing slowly throughout. Beta1 applies.
#   Phase 1 (t ≥ 14): full lockdown 14 December 2020 (week 15 of wave).
#                     Rapid decline follows. Beta2 applies.
#   No reopening within this wave window → Beta3 = Beta2 (set identically).

npi_df    <- data.frame(week      = 0:22,
                        npi_phase = c(rep(0, 14), rep(1, 9)))
npi_covar <- covariate_table(npi_df, times = "week")

# ─────────────────────────────────────────────────────────────────────────────
# 3.  PROCESS MODEL
# ─────────────────────────────────────────────────────────────────────────────

seir_step_npi <- Csnippet("
  double eff_beta;
  if      (npi_phase < 0.5) eff_beta = Beta1;
  else if (npi_phase < 1.5) eff_beta = Beta2;
  else                       eff_beta = Beta3;

  double dN_SE = rbinom(S, 1 - exp(-eff_beta * I / N * dt));
  double dN_EI = rbinom(E, 1 - exp(-mu_EI * dt));
  double dN_IR = rbinom(I, 1 - exp(-mu_IR * dt));

  S -= dN_SE;
  E += dN_SE - dN_EI;
  I += dN_EI - dN_IR;
  R += dN_IR;
  H += dN_IR;
")

# ─────────────────────────────────────────────────────────────────────────────
# 4.  OBSERVATION MODEL
# ─────────────────────────────────────────────────────────────────────────────

covid_dmeas <- Csnippet("lik = dnbinom_mu(reports, k, rho * H, give_log);")
covid_rmeas <- Csnippet("reports = rnbinom_mu(k, rho * H);")

# ─────────────────────────────────────────────────────────────────────────────
# 5.  INITIAL CONDITIONS
# ─────────────────────────────────────────────────────────────────────────────
# Wave 2 starts with a large active epidemic (12,710 reported in week 1).
# Data-implied I0 = week1_reports / (rho × mu_IR) = 12710 / (0.18 × 2.41) ≈ 29299
# Data-implied E0 = I0 × (mu_IR / mu_EI) = 29299 × (2.41/1.37) ≈ 51541
# Using I=1 would cause instant extinction — same problem as Wave 1 but worse.

seir_rinit <- Csnippet("
  double I_init = 29299.0;
  double E_init = 51541.0;
  I = nearbyint(I_init);
  E = nearbyint(E_init);
  S = nearbyint(eta * N) - I - E;
  R = nearbyint((1.0 - eta) * N);
  H = 0.0;
")

# ─────────────────────────────────────────────────────────────────────────────
# 6.  BUILD pomp OBJECT
# ─────────────────────────────────────────────────────────────────────────────
# Estimated: Beta1, Beta2, Beta3, rho, k  (5 parameters)
# Fixed:     N, mu_EI, mu_IR, eta
#   — mu_IR fixed: collinear with Beta1 along R0=Beta1/mu_IR ridge
#   — mu_EI fixed: well-characterised (Lauer 2020); reduces parameter space
#   — eta fixed:   collinear with rho; set to 0.95 from W1 seroprevalence

params_est <- c("Beta1", "Beta2", "Beta3", "rho", "k")

covid_w2 <- meas_w2 |>
  pomp(
    times      = "week",
    t0         = 0,
    rprocess   = euler(seir_step_npi, delta.t = 1/7),
    rinit      = seir_rinit,
    rmeasure   = covid_rmeas,
    dmeasure   = covid_dmeas,
    covar      = npi_covar,
    partrans   = parameter_trans(
      log   = c("Beta1", "Beta2", "Beta3", "k"),
      logit = c("rho")
    ),
    paramnames = c("N","Beta1","Beta2","Beta3","mu_EI","mu_IR","eta","k","rho"),
    statenames = c("S","E","I","R","H"),
    accumvars  = "H"
  )

# ─────────────────────────────────────────────────────────────────────────────
# 7.  STARTING PARAMETERS
# ─────────────────────────────────────────────────────────────────────────────
# Beta1 = 2.8/wk  → R0 = 1.16; consistent with observed growth rate ≈ 0.37/wk
#                   Slow autumn growth driven by waning compliance + partial NPIs
# Beta2 = 2.2/wk  → Rt = 0.91; post-lockdown decline (Dec 2020)
# Beta3 = 2.2/wk  → same as Beta2; no reopening phase within this wave window
# rho   = 0.18    → expanded testing vs Wave 1; ~18% of infections reported
# eta   = 0.95    → FIXED; ~5% immune from Wave 1 (seroprevalence estimate)
# mu_EI = 1.37    → FIXED: 7/5.1d incubation (Lauer et al. 2020)
# mu_IR = 2.41    → FIXED: 7/2.9d infectious (He et al. 2020)

theta_start <- c(
  N     = 17400000,
  Beta1 = 2.8,
  Beta2 = 2.2,
  Beta3 = 2.2,
  mu_EI = 1.37,
  mu_IR = 2.41,
  eta   = 0.95,
  rho   = 0.18,
  k     = 10
)

coef(covid_w2) <- theta_start

# ─────────────────────────────────────────────────────────────────────────────
# 8.  SIMULATION CHECK
# ─────────────────────────────────────────────────────────────────────────────
# NOTE: Wave 2 simulations will NOT look like a classic epidemic bell curve.
# Blue lines start high (~12k) and rise slowly to ~76k over 14 weeks.
# That is correct — the epidemic was already active when the wave window begins.
# What to check: right order of magnitude, peak around week 14-15, no explosion.

set.seed(2024)
covid_w2 |>
  simulate(nsim = 30, format = "data.frame", include.data = TRUE) |>
  ggplot(aes(x = week, y = reports, group = .id,
             colour    = (.id == "data"),
             linewidth = (.id == "data"),
             alpha     = (.id == "data"))) +
  geom_line() +
  scale_colour_manual(values = c("TRUE"="black","FALSE"="#2166ac"), guide="none") +
  scale_linewidth_manual(values = c("TRUE"=1.0,"FALSE"=0.3), guide="none") +
  scale_alpha_manual(values = c("TRUE"=1.0,"FALSE"=0.4), guide="none") +
  scale_y_continuous(labels = scales::comma) +
  labs(title    = "Wave 2: Simulation check",
       subtitle = "Lines start high (epidemic already active) — peak expected ~week 14-15",
       x = "Week", y = "Weekly reported cases") +
  theme_bw(base_size = 11)

# ─────────────────────────────────────────────────────────────────────────────
# 9.  PRIOR SPECIFICATION
# ─────────────────────────────────────────────────────────────────────────────
# All priors on transformed (log / logit) scale.
# Beta1: log-N(1.03, 0.40²)  → centred on log(2.8), covers Rt ∈ (0.8, 2.5)
# Beta2: log-N(0.79, 0.50²)  → centred on log(2.2), covers post-lockdown range
# Beta3: log-N(0.79, 0.50²)  → same centre as Beta2 (no reopening expected)
# rho:   logit-N(-1.52, 0.60²) → centred on logit(0.18), 95% mass on (0.07, 0.38)
# k:     log-N(2.30, 0.80²)  → centred on log(10)

covid_dprior <- Csnippet("
  double lB1  = log(Beta1);
  double lB2  = log(Beta2);
  double lB3  = log(Beta3);
  double lrho = log(rho / (1.0 - rho));
  double lk   = log(k);

  lik = dnorm(lB1,  1.03, 0.40, 1)
      + dnorm(lB2,  0.79, 0.50, 1)
      + dnorm(lB3,  0.79, 0.50, 1)
      + dnorm(lrho,-1.52, 0.60, 1)
      + dnorm(lk,   2.30, 0.80, 1);

  if (!give_log) lik = exp(lik);
")

covid_w2 <- pomp(covid_w2, dprior = covid_dprior,
                 paramnames = c("N","Beta1","Beta2","Beta3",
                                "mu_EI","mu_IR","eta","k","rho"))

# ─────────────────────────────────────────────────────────────────────────────
# 10.  PARTICLE FILTER DIAGNOSTIC
# ─────────────────────────────────────────────────────────────────────────────

Np_use <- 3000

set.seed(999)
ll_check <- replicate(10, logLik(pfilter(covid_w2, Np = Np_use)))
ll_check <- ll_check[is.finite(ll_check)]
cat(sprintf("\npfilter (Np=%d): mean=%.2f  SD=%.3f\n",
            Np_use, mean(ll_check), sd(ll_check)))
if (sd(ll_check) > 0.5)
  cat("  CAUTION: SD > 0.5 — increase Np_use before running PMCMC\n") else
  cat("  OK — proceed to PMCMC\n")

# ─────────────────────────────────────────────────────────────────────────────
# 11.  PHASE 1 PMCMC — DIAGONAL PROPOSAL
# ─────────────────────────────────────────────────────────────────────────────

pmcmc_p1 <- pmcmc(
  covid_w2,
  Nmcmc    = 5000,
  Np       = Np_use,
  proposal = mvn_diag_rw(c(Beta1=0.15, Beta2=0.15, Beta3=0.15,
                            rho=0.15, k=0.20)^2)
)
acc_p1 <- pmcmc_p1@accepts / 5000
cat(sprintf("Phase 1 acceptance rate: %.3f\n", acc_p1))

# Acceptance rate is too high, but it will be compensated in next step

# ─────────────────────────────────────────────────────────────────────────────
# 12.  PHASE 2 PMCMC — ADAPTIVE MVN PROPOSAL
# ─────────────────────────────────────────────────────────────────────────────

chain_p1  <- as.matrix(as.data.frame(traces(pmcmc_p1)))
post_cov  <- cov(chain_p1[2001:5000, params_est])
opt_scale <- (2.38^2) / length(params_est)

pmcmc_p2 <- pmcmc(
  pmcmc_p1,
  Nmcmc    = 15000,
  Np       = Np_use,
  proposal = mvn_rw(opt_scale * post_cov)
)
acc_p2 <- pmcmc_p2@accepts / 15000
cat(sprintf("Phase 2 acceptance rate: %.3f  (target 0.20–0.40)\n", acc_p2))

# llançat a les 11:18 finalitzat 12:30

# ─────────────────────────────────────────────────────────────────────────────
# 13.  PRODUCTION CHAIN AND DIAGNOSTICS
# ─────────────────────────────────────────────────────────────────────────────

chain_p2   <- as.matrix(as.data.frame(traces(pmcmc_p2)))
prod_chain <- as.mcmc(chain_p2[2001:nrow(chain_p2), params_est])

cat("ESS:\n"); print(round(effectiveSize(prod_chain)))
plot(prod_chain, ask = FALSE)

# ─────────────────────────────────────────────────────────────────────────────
# 14.  POSTERIOR SUMMARIES
# ─────────────────────────────────────────────────────────────────────────────

post_df <- as.data.frame(prod_chain) |>
  mutate(R0      = Beta1 / theta_start["mu_IR"],
         Rt_lock = Beta2 / theta_start["mu_IR"],
         Rt_open = Beta3 / theta_start["mu_IR"])

cat("\nPosterior means:\n"); print(round(colMeans(post_df[, params_est]), 4))
cat("\n95% CIs:\n"); print(round(apply(post_df[, params_est], 2, quantile, c(0.025, 0.975)), 4))
for (v in c("R0","Rt_lock","Rt_open"))
  cat(sprintf("  %s: %.2f [%.2f, %.2f]\n", v,
              mean(post_df[[v]]),
              quantile(post_df[[v]], 0.025),
              quantile(post_df[[v]], 0.975)))

# ─────────────────────────────────────────────────────────────────────────────
# 15.  POSTERIOR DENSITY PLOTS
# ─────────────────────────────────────────────────────────────────────────────

post_df |>
  select(all_of(params_est)) |>
  pivot_longer(everything(), names_to = "parameter", values_to = "value") |>
  ggplot(aes(x = value)) +
  geom_histogram(aes(y = after_stat(density)), bins=50, fill="#2166ac", alpha=0.55) +
  geom_density(colour = "black", linewidth = 0.8) +
  facet_wrap(~parameter, scales = "free", nrow = 2) +
  labs(title = "Wave 2: Posterior marginal distributions",
       x = "Parameter value", y = "Density") +
  theme_bw(base_size = 11)

# ─────────────────────────────────────────────────────────────────────────────
# 16.  PAIRWISE SCATTER
# ─────────────────────────────────────────────────────────────────────────────

ggpairs(post_df |> select(all_of(params_est)),
        lower = list(continuous = wrap("points", alpha=0.04, size=0.4)),
        upper = list(continuous = wrap("cor", size=3)),
        diag  = list(continuous = wrap("densityDiag"))) +
  theme_bw(base_size = 9) +
  labs(title = "Wave 2: Pairwise posterior scatter")

# ─────────────────────────────────────────────────────────────────────────────
# 17.  POSTERIOR PREDICTIVE CHECK
# ─────────────────────────────────────────────────────────────────────────────

set.seed(42)
pp_sims <- lapply(sample(nrow(post_df), 300), function(i) {
  th            <- coef(covid_w2)
  th[params_est] <- as.numeric(post_df[i, params_est])
  simulate(covid_w2, params=th, nsim=1, format="data.frame") |>
    mutate(draw = i)
})

pp_band <- bind_rows(pp_sims) |>
  group_by(week) |>
  summarise(lo=quantile(reports,0.025,na.rm=TRUE),
            hi=quantile(reports,0.975,na.rm=TRUE),
            med=median(reports,na.rm=TRUE), .groups="drop")

ggplot() +
  geom_ribbon(data=pp_band, aes(x=week,ymin=lo,ymax=hi),
              fill="#2166ac", alpha=0.25) +
  geom_line(data=pp_band, aes(x=week,y=med),
            colour="#2166ac", linewidth=1) +
  geom_point(data=as.data.frame(covid_w2)|>select(week,reports),
             aes(x=week,y=reports), colour="black", size=2) +
  scale_y_continuous(labels=scales::comma) +
  labs(title    = "Wave 2: Posterior predictive check",
       subtitle = "Shaded=95% interval; line=median; dots=data",
       x="Week", y="Weekly reported cases") +
  theme_bw(base_size=11)
