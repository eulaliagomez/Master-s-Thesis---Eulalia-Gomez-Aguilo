# =============================================================================
# sensitivity_diagnostics_wave2.R
#
# Posterior densities, pairwise scatter, and PPC for all Wave 2 sensitivity
# runs. Saves to results/diagnostics/ with one subfolder per run.
#
...
wave_name  <- "wave2"
params_est <- cfg$params_est   # Beta1 Beta2 Beta3 Beta4 rho_y rho_o
dom_eigen  <- 4.074
...
baseline_chain <- readRDS("results/baseline_wave2.rds")
...
get_degraded_data <- function(run) {
  ...
  if (run$type == "systematic") {
    ...
    deg_seed <- as.integer(paste0(2, round(run$f * 100)))
...
make_density <- function(post_df_run, run_label) {
  ...
  bind_rows(deg_long, base_long) |>
    ...
    scale_fill_manual(values=c("Baseline"="#2166ac","Degraded"="#d73027"),
                      name=NULL) +
    scale_colour_manual(values=c("Baseline"="#2166ac","Degraded"="#d73027"),
                        name=NULL) +
    labs(title   = sprintf("Wave 2 posterior densities: %s", run_label),
...
make_pairs <- function(post_df_run, run_label) {
  tryCatch(
    ggpairs(
      ...
      labs(title=sprintf("Wave 2 pairwise posterior: %s", run_label)),
    error = function(e) {
      ggplot() +
...
make_ppc <- function(po, post_df_run, run_label, n_draws=500) {
  ...
  ggplot() +
    ...
    geom_point(data=obs, aes(x=week, y=cases),
               colour="black", size=2, shape=16) +
    facet_wrap(~series, scales="free_y", nrow=3) +
    scale_y_continuous(labels=scales::comma) +
    labs(title   = sprintf("Wave 2 PPC: %s", run_label),
...
for (run in runs) {
  ...
  rds_path <- file.path("results",
                        sprintf("wave2_%s.rds", run$id))
  if (!file.exists(rds_path)) {
    cat("  SKIP — not found:", rds_path, "\n\n"); next
  }
  ...
  # Density
  cat("  Density plot...\n")
  p <- make_density(post_df_run, run$label)
  ggsave(file.path(out_dir,"posterior_densities.pdf"), p, width=14, height=7)
  ggsave(file.path(out_dir,"posterior_densities.png"), p, width=14, height=7, dpi=150)

  # Pairs
  cat("  Pairs plot...\n")
  p <- make_pairs(post_df_run, run$label)
  ggsave(file.path(out_dir,"pairwise_scatter.pdf"), p, width=12, height=12)
  ggsave(file.path(out_dir,"pairwise_scatter.png"), p, width=12, height=12, dpi=150)

  # PPC
  cat("  PPC...\n")
  p <- make_ppc(po, post_df_run, run$label)
...
cat(strrep("-", 60), "\n")
cat("Processing: Wave 2 Baseline\n")

out_dir_base <- "results/diagnostics/wave2_baseline"
...
p_dens <- baseline_df |>
  ...
  labs(title="Wave 2 posterior densities: Baseline",
       x="Parameter value", y="Density") +
  theme_bw(base_size=11)
ggsave(file.path(out_dir_base,"posterior_densities.pdf"), p_dens, width=14, height=7)
ggsave(file.path(out_dir_base,"posterior_densities.png"), p_dens, width=14, height=7, dpi=150)

# Pairs
p_pairs <- make_pairs(baseline_df, "Baseline (clean data)")
ggsave(file.path(out_dir_base,"pairwise_scatter.pdf"), p_pairs, width=12, height=12)
ggsave(file.path(out_dir_base,"pairwise_scatter.png"), p_pairs, width=12, height=12, dpi=150)
...
set.seed(42)
sims <- lapply(sample(nrow(baseline_df), 500), function(i) {
  th             <- coef(po_base)
  th[params_est] <- as.numeric(baseline_df[i, params_est])
  simulate(po_base, params=th, nsim=1, format="data.frame") |>
    mutate(draw=i, reports_total=reports_young+reports_old)
...
p_ppc <- ggplot() +
  geom_ribbon(data=pp_band,
              aes(x=week, ymin=lo, ymax=hi, fill=series), alpha=0.25) +
  geom_line(data=pp_band,
            aes(x=week, y=med, colour=series), linewidth=1) +
  geom_point(data=obs_long,
             aes(x=week, y=cases, colour=series), size=2, shape=16) +
  facet_wrap(~series, scales="free_y", nrow=3) +
  scale_colour_manual(values=series_cols, guide="none") +
  scale_fill_manual(values=series_cols, guide="none") +
  scale_y_continuous(labels=scales::comma) +
  labs(title="Wave 2 PPC: Baseline (clean data)",
       subtitle="Shaded=95% interval | Line=median | Dots=observed",
       x="Week", y="Weekly reported cases") +
  theme_bw(base_size=11)
ggsave(file.path(out_dir_base,"ppc.pdf"), p_ppc, width=10, height=10)
ggsave(file.path(out_dir_base,"ppc.png"), p_ppc, width=10, height=10, dpi=150)

cat("  Baseline saved to:", out_dir_base, "\n\n")
cat(strrep("=", 60), "\n")
cat("ALL WAVE 2 DIAGNOSTICS COMPLETE\n")
cat("Output in results/diagnostics/wave2_*/\n")
