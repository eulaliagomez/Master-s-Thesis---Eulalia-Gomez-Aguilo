# =============================================================================
# sensitivity_diagnostics_wave4.R
# Posterior densities, pairwise scatter, PPC for Wave 4 sensitivity runs.
# setwd("C:/Users/Usuario/Desktop/CBS - Internship/Analysis/DATA")
# source("sensitivity_diagnostics_wave4.R")
# =============================================================================

library(tidyverse); library(pomp); library(coda); library(GGally)
...
series_cols <- c("Young (<60)"="#2166ac","Old (\u226560)"="#d73027","Total"="#1a7a1a")
...
baseline_chain <- readRDS("results/baseline_wave4.rds")
...
get_degraded_data <- function(run) {
  csv_path <- file.path("results/degraded_data", sprintf("wave4_%s.csv", run$id))
  ...
  if (run$type=="stochastic") {
    return(degrade_stochastic(meas_clean, run$f, seed=as.integer(paste0(4,round(run$f*100)))))
  }
...
make_density <- function(post_df_run, run_label) {
  bind_rows(
    post_df_run |> select(all_of(params_est)) |>
      pivot_longer(everything(), names_to="parameter", values_to="value") |> mutate(source="Degraded"),
    baseline_df  |> select(all_of(params_est)) |>
      pivot_longer(everything(), names_to="parameter", values_to="value") |> mutate(source="Baseline")
    ...
    labs(title=sprintf("Wave 4 posterior densities: %s", run_label),
         subtitle="Red=degraded | Blue=baseline", x="Parameter value", y="Density") +
...
make_pairs <- function(post_df_run, run_label) {
  tryCatch(
    ggpairs(post_df_run |> select(all_of(params_est)),
            ...
            diag =list(continuous=wrap("densityDiag",colour="#d73027",fill="#d73027",alpha=0.4))) +
      theme_bw(base_size=9) + labs(title=sprintf("Wave 4 pairwise: %s", run_label)),
    error=function(e) ggplot()+annotate("text",x=0.5,y=0.5,label=paste("Failed:",e$message),size=4)+theme_void()
  )
}
...
make_ppc <- function(po, post_df_run, run_label, n_draws=500) {
  ...
  bands <- bind_rows(c(sims_deg, sims_base)) |>
    pivot_longer(c(reports_young,reports_old,reports_total), names_to="series", values_to="cases") |>
    mutate(series=recode(series,"reports_young"="Young (<60)","reports_old"="Old (\u226560)","reports_total"="Total"),
  ...
  ggplot() +
    ...
    labs(title=sprintf("Wave 4 PPC: %s", run_label),
         subtitle="Red=degraded | Blue dashed=baseline | Black=observed",
...
for (run in runs) {
  ...
  rds_path <- file.path("results", sprintf("wave4_%s.rds", run$id))
  if (!file.exists(rds_path)) { cat("  SKIP\n\n"); next }
  res <- readRDS(rds_path); post_df_run <- as.data.frame(res$chain)
  ...
  } else { po <- build_pomp(cfg, meas_deg) }
...
# Baseline
cat(strrep("-",60),"\n"); cat("Processing: Wave 4 Baseline\n")
out_dir_base <- "results/diagnostics/wave4_baseline"
...
p <- baseline_df |> select(all_of(params_est)) |>
  ...
  facet_wrap(~parameter, scales="free", nrow=2,
             labeller=as_labeller(c(Beta1="beta[1]",Beta2="beta[2]",Beta3="beta[3]",
                                    rho_y="rho[y]",rho_o="rho[o]"), label_parsed)) +
  labs(title="Wave 4 posterior densities: Baseline", x="Parameter value", y="Density") +
  theme_bw(base_size=11)
...
cat("  Baseline saved.\n")
cat(strrep("=",60),"\nWAVE 4 DIAGNOSTICS COMPLETE\nOutput: results/diagnostics/wave4_*/\n")
