# =============================================================================
# sensitivity_plots_wave2.R
#
# Sensitivity analysis plots for Wave 2.
# Saves all figures to results/wave2/ to avoid overwriting Wave 1 figures.
#
# setwd("C:/Users/Usuario/Desktop/CBS - Internship/Analysis/DATA")
# source("sensitivity_plots_wave2.R")
# =============================================================================

library(tidyverse)

if (!dir.exists("results/wave2")) dir.create("results/wave2", recursive = TRUE)

metrics_df <- readRDS("results/wave2_metrics.rds") |>
  mutate(degradation = tolower(as.character(degradation)))
summary_df <- readRDS("results/wave2_summary.rds") |>
  mutate(degradation = tolower(as.character(degradation)))
...
# Wave 2 has 6 parameters (Beta1-Beta4 + rho_y + rho_o)
param_levels <- c("Beta1","Beta2","Beta3","Beta4","rho_y","rho_o")
...
save_fig <- function(p, name, w = 12, h = 8) {
  ggsave(file.path("results/wave2", paste0(name, ".pdf")), p, width=w, height=h)
  ggsave(file.path("results/wave2", paste0(name, ".png")), p, width=w, height=h, dpi=150)
  cat(name, "saved\n")
}
...
fig1 <- fig1_data |>
  ...
  scale_colour_manual(
    values = c("systematic"="#154273","stochastic"="#00a1d5","aggregate"="#e17000"),
    name = "Degradation type") +
  labs(title    = "Wave 2: Posterior means and 95% CIs under data degradation",
       subtitle = "Dashed = baseline mean | Dotted = baseline 95% CI",
       x = "Scenario", y = "Parameter value") +
  theme_bw(base_size=11) +
  theme(legend.position="bottom", panel.grid.minor=element_blank())
save_fig(fig1, "fig1_posterior_means", w=14, h=8)
...
fig2 <- fig2_data |>
  ...
  scale_fill_manual(
    values=c("systematic"=col_sys,"stochastic"=col_sto,"aggregate"=col_agg),
    name=NULL) +
  labs(title="Wave 2: CI width ratio vs baseline",
...
save_fig(fig2, "fig2_ci_ratio", w=14, h=8)
...
fig3 <- fig2_data |>
  ...
  scale_fill_manual(
    values=c("systematic"=col_sys,"stochastic"=col_sto,"aggregate"=col_agg),
    name=NULL) +
  labs(title="Wave 2: Posterior mean shift vs baseline",
  ...
  theme(legend.position="none",
        axis.text.x=element_text(angle=45, hjust=1, size=8),
        panel.grid.minor=element_blank())
save_fig(fig3, "fig3_mean_shift", w=14, h=8)
...
sys_data <- summary_df |>
  ...
  mutate(
    ...
    predicted      = ifelse(grepl("rho", parameter), 1-(1-f), 1.0),
    # corrected: rho*(1-f)/rho = (1-f), ratio goes down
...
fig4 <- ggplot() +
  ...
  geom_line(data=sys_data,
            aes(x=f, y=observed_ratio, colour=parameter, group=parameter),
            linewidth=0.9) +
  geom_point(data=sys_data,
             aes(x=f, y=observed_ratio, colour=parameter, shape=parameter),
             size=3) +
  scale_x_continuous(breaks=c(0.10,0.30,0.50),
                     labels=c("10%","30%","50%")) +
  scale_colour_brewer(palette="Set1", name="Parameter") +
  scale_shape_manual(values=c(16,17,15,3,4,8), name="Parameter") +
  scale_linetype_manual(
    values=c("Detection rate"="dashed","Transmission rate"="dotted"),
    name="Theoretical\nprediction") +
  labs(title="Wave 2: Theory vs observed — systematic reduction",
       subtitle="Dashed grey = predicted rho ratio: (1-f) | Dotted grey = predicted beta: no change",
...
fig5 <- ggplot(fig5_data, aes(x=scenario, y=parameter, fill=coverage)) +
  ...
  scale_y_discrete(labels=c(
    ...
    Beta3=expression(beta[3]), Beta4=expression(beta[4]),
    rho_y=expression(rho[y]), rho_o=expression(rho[o]))) +
  labs(title="Wave 2: Coverage heatmap",
       subtitle="\u2713 = robust | \u2717 = shift exceeds baseline uncertainty",
       x="Scenario", y="Parameter") +
  theme_bw(base_size=11) +
  theme(panel.grid=element_blank(), legend.position="right",
        axis.text.x=element_text(size=9))
save_fig(fig5, "fig5_coverage", w=12, h=5)

# ── Figure 6: Systematic vs stochastic ───────────────────────────────────────
...
fig6 <- ggplot(fig6_data,
               aes(x=f, y=ci_width_ratio, colour=degradation,
                   linetype=degradation, group=degradation)) +
  geom_hline(yintercept=1, linetype="dashed", colour="grey70", linewidth=0.4) +
  geom_line(linewidth=1.0) +
  geom_point(size=3) +
  facet_wrap(~parameter, nrow=2, labeller=param_labeller) +
  scale_colour_manual(values=c("Systematic"=col_sys,"Stochastic"=col_sto),
                      name=NULL) +
  scale_linetype_manual(values=c("Systematic"="solid","Stochastic"="dashed"),
                        name=NULL) +
  scale_x_continuous(breaks=c(0.10,0.30,0.50),
                     labels=c("10%","30%","50%")) +
  labs(title="Wave 2: Systematic vs stochastic — CI width ratio",
       subtitle="Gap between lines = extra uncertainty from binomial noise alone",
       x="Fraction removed (f)", y="CI width ratio") +
  theme_bw(base_size=11) +
  theme(legend.position="bottom", panel.grid.minor=element_blank())
save_fig(fig6, "fig6_sys_vs_sto", w=14, h=8)

cat("\nAll Wave 2 figures saved to results/wave2/\n")
