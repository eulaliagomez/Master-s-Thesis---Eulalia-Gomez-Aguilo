# =============================================================================
# sensitivity_plots_wave3.R
#
# Sensitivity analysis plots for Wave 3.
# Saves all figures to results/wave3/
#
...
if (!dir.exists("results/wave3")) dir.create("results/wave3", recursive=TRUE)

metrics_df <- readRDS("results/wave3_metrics.rds") |>
  mutate(degradation = tolower(as.character(degradation)))
summary_df <- readRDS("results/wave3_summary.rds") |>
  mutate(degradation = tolower(as.character(degradation)))
...
# Wave 3 has 5 parameters (Beta1 Beta2 Beta3 rho_y rho_o)
...
save_fig <- function(p, name, w=12, h=8) {
  ggsave(file.path("results/wave3", paste0(name, ".pdf")), p, width=w, height=h)
  ggsave(file.path("results/wave3", paste0(name, ".png")), p, width=w, height=h,
         dpi=150)
  cat(name, "saved\n")
}
...
fig1 <- fig1_data |>
  ...
  labs(title="Wave 3: Posterior means and 95% CIs under data degradation",
...
fig2 <- fig2_data |>
  ...
  scale_fill_manual(
    values=c("systematic"=col_sys,"stochastic"=col_sto,"aggregate"=col_agg),
    name=NULL) +
  labs(title="Wave 3: CI width ratio vs baseline",
...
fig3 <- fig2_data |>
  ...
  scale_fill_manual(
    values=c("systematic"=col_sys,"stochastic"=col_sto,"aggregate"=col_agg),
    name=NULL) +
  labs(title="Wave 3: Posterior mean shift vs baseline",
...
fig4 <- ggplot() +
  ...
  scale_linetype_manual(
    ...
    name="Theoretical\nprediction") +
  labs(title="Wave 3: Theory vs observed — systematic reduction",
...
fig5 <- ggplot(fig5_data, aes(x=scenario, y=parameter, fill=coverage)) +
  ...
  labs(title="Wave 3: Coverage heatmap",
...
fig6 <- ggplot(fig6_data,
  ...
  scale_x_continuous(breaks=c(0.10,0.30,0.50),
                     labels=c("10%","30%","50%")) +
  labs(title="Wave 3: Systematic vs stochastic — CI width ratio",
...
cat("\nAll Wave 3 figures saved to results/wave3/\n")
